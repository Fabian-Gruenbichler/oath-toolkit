...
static time_t parse_time (const char *p, const time_t now)
{
...
static void verbose_hotp (uint64_t moving_factor)
{
    printf ("Start counter: 0x%" PRIX64 " (%" PRIu64 ")\n\n",
            moving_factor, moving_factor);
}

static void verbose_totp (time_t t0, time_t time_step_size, time_t when)
{
    struct tm tmp;
    char outstr[200];
...
main (int argc, char *argv[])
{
    ...
    switch(mode) {
        ...
        case OATH_OCRA:
            ...
            rc = oath_hex2bin(args_info.challenges_arg,challenges_bin,&bin_length);
            if(rc != OATH_OK)
                error (EXIT_FAILURE,0,
            ...
            if (generate_otp_p (args_info.inputs_num))
            {
                ...
                if(rc != OATH_OK)
                    error (EXIT_FAILURE, 0,
                            "generating OCRA value failed (%d)", rc);
                printf("%s\n",otp);
            } 
            else if(validate_otp_p (args_info.inputs_num))
            {
                ...
                if(rc != OATH_OK)
                    error (EXIT_FAILURE, 0,
                            "validating OCRA value failed (%d)", rc);
                printf("%d\n",rc);
